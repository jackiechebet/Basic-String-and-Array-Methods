...
let userData = {
    ...
    currentSong: null,
    songCurrentTime: 0,
};//object to keep track of the song: current song and the time of the current song
 const playSong = (id) => {
...
const  playNextSong = () => {
  if (userData?.currentSong === null) {
    ...
    // to retrieve the next song in the playlist.
...
const  highlightCurrentSong = () => {
  ...
  playlistSongElements.forEach((songEl) => {
    ...
    // removes "aria-current" for each of the songs.
  });
  if (songToHighlight) {
...
playlistSongs.innerHTML = songsHTML;
   //his will insert the li element you just created into the ul element in the already provided HTML file.  
};
const setPlayButtonAccessibleText = () => {
  const song = userData?.currentSong || userData?.songs[0].id;
  playButton.setAttribute("aria-label", song?.title ? `Play ${song.title}` : "Play");
};//will set the aria-label attribute to the current song, or to the first song in the playlist. And if the playlist is empty, it sets the aria-label to "Play".
